...
% shift the whole output to the new position
%d_output=d_output(:,6:end); 
d_output=d_output(:,11:end);
...
%phim=10; % phase factor for detuning

[d1, d2]=size(d_train); %size of the training data
N=d1*11; % The size of the x_train matrix
...
x_train=zeros(N+1,T-10); % The construction of the x matrix
...
%display_x_train=['Standard deviation of x_train is: ',num2str(std(x_train(:)))];
%disp(display_x_train);

%x_train(1:N,:)=prevs_and_futr(d_train(:,1:T));
x_train(1:N,:)=lnr(d_train(:,1:T));

%x_train=abs(x_train+d_bias).^2;

x_train=(x_train-mean(x_train(:)))./std(x_train(:));

x_train(N+1,:)=ones(1,T-10); % set the last line of the matrix x as all ones.
...
eta0=0.00005; % time step for the matri M's iterations
eta0_lsq = 0.00005;
while k<=STEP
    eta = eta0*(1-k/STEP);
...
t2=1010000;
dt=t2-t1+1;
x_test=zeros(N+1,dt-10);
u_test=d_train(:,t1:t2);

yy_test=d_output(:,t1:t2-10);% desired output data for the test
...
%x_test(1:N,:)=prevs_and_futr(u_test);
x_test(1:N,:)=lnr(u_test);

%x_test=abs(x_test+d_bias).^2;
x_test=(x_test-mean(x_test(:)))./std(x_test(:));
x_test(N+1,:)=ones(1,dt-10); % set the last line of the matrix x as all ones
...
%% Plot the figure title and legend with classification error of the test data


titlename=['N = ',num2str(N),', Test Error = ',num2str(class_error_test), ', Initial learning rate $\eta =$',num2str(eta0)];
title(titlename,'Interpreter','LaTex');
